...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/09/09 16:55:30 by smrabet           #+#    #+#             */
/*   Updated: 2017/09/09 16:55:32 by smrabet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_col_n_pos(t_mlx *mlx, t_obj *link, int k)
{
	...
	if (ft_strncmp(mlx->scene[k + 2], "		col(", 6) != 0 || \
		...
		return (-1);
	link->col.red = ft_atoi_type(mlx->scene[k + 2], &i);
	link->col.green = ft_atoi_type(mlx->scene[k + 2], &i);
	link->col.blue = ft_atoi_type(mlx->scene[k + 2], &i);
	if (mlx->scene[k + 2][i] != ')')
		return (-1);
	i = 6;
	link->pos.x = ft_atoi_type(mlx->scene[k + 3], &i);
	link->pos.y = ft_atoi_type(mlx->scene[k + 3], &i);
	link->pos.z = ft_atoi_type(mlx->scene[k + 3], &i);
	if (mlx->scene[k + 3][i] != ')')
		return (-1);
	return (0);
}
...
int			ft_add_plane(t_mlx *mlx, int *k, int i)
{
	...
	if (ft_col_n_pos(mlx, link, *k) == -1 || \
		...
		return (-1);
	link->rot.x = (float)ft_atoi_type(mlx->scene[*k + 4], &i);
	link->rot.y = (float)ft_atoi_type(mlx->scene[*k + 4], &i);
	link->rot.z = (float)ft_atoi_type(mlx->scene[*k + 4], &i);
	if (mlx->scene[*k + 4][i] != ')')
		return (-1);
	link->next = NULL;
...
int			ft_add_cyl_cone(t_mlx *mlx, int *k, int t, int i)
{
	...
	if (ft_strncmp(mlx->scene[*k + 5], "		size(", 7) != 0)
		return (-1);
	i = 7;
	link->size = ft_atoi_type(mlx->scene[*k + 5], &i);
	if (mlx->scene[*k + 5][i] != ')')
		return (-1);
	link->next = NULL;
	mlx->obj = ft_add_object_link(mlx, link);
...
int			ft_add_sphere(t_mlx *mlx, int *k, int i)
{
	...
	if (ft_strncmp(mlx->scene[*k + 4], "		size(", 7) != 0)
		return (-1);
	link->size = ft_atoi_type(mlx->scene[*k + 4], &i);
	if (mlx->scene[*k + 4][i] != ')')
		return (-1);
	link->next = NULL;
	mlx->obj = ft_add_object_link(mlx, link);
