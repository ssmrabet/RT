...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/09/09 16:58:08 by smrabet           #+#    #+#             */
/*   Updated: 2017/09/09 16:58:10 by smrabet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				ft_parameters(int keycode, t_mlx *mlx)
{
	if (keycode == 36 && (mlx->selection = 0) > -1)
		return (0);
	else if (keycode == KEY_PLUS && TMP->type != 3)
		TMP->size = (TMP->size < 10000) ? TMP->size + 5 : TMP->size;
	else if (keycode == KEY_MOIN && TMP->type != 3)
...
{
	mlx->pos = ft_strjoin_type("pos: ", (
	ft_strjoin_type(ft_itoa(TMP->pos.x), (
	ft_strjoin_type(" ", (
	ft_strjoin_type(ft_itoa(TMP->pos.y), (
	ft_strjoin_type(" ", ft_itoa(TMP->pos.z), 'R')), 'B')), 'R')), 'B')), 'R');
	if (TMP->type != 4)
	{
		mlx->rot = ft_strjoin_type("rot: ", (
		ft_strjoin_type(ft_ftoa(TMP->rot.x), (
		ft_strjoin_type(" ", (
		ft_strjoin_type(ft_ftoa(TMP->rot.y), (
		ft_strjoin_type(" ", ft_ftoa(TMP->rot.z), 'R')), 'B')), 'R')), 'B')),
			'R');
	}
}
...
void			ft_string_put(t_mlx *mlx)
{
	...
	s = (TMP->type != 3) ? ft_strjoin_type("size: ", ft_itoa(TMP->size), 'R')
		: NULL;
	ft_tmp_string(mlx);
