...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/09/09 16:55:14 by smrabet           #+#    #+#             */
/*   Updated: 2017/09/09 16:55:16 by smrabet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_start(char *s, int *fd)
{
	...
	{
		ft_putendl(ft_strjoin_type("error: ", strerror(errno), 'N'));
		return (-1);
	}
	return (1);
}
...
static char		*ft_getbuf(char *s, int *nbl)
{
	...
	while (get_next_line(fd, &line) > 0 && (*nbl)++ > -1)
		buf = ft_strjoin_type(buf, ft_strjoin_type(line, "\n", 'L'), 'B');
	if (close(fd) == -1)
	{
		ft_putendl(ft_strjoin_type("error: ", strerror(errno), 'N'));
		return (NULL);
