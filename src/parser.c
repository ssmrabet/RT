...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/09/09 16:55:47 by smrabet           #+#    #+#             */
/*   Updated: 2017/09/09 16:55:50 by smrabet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_checkscene(t_mlx *mlx, int i)
{
	...
	if (ft_strncmp(mlx->scene[2], "	cam_pos(", 9) != 0)
		return (-1);
	mlx->cam_pos.x = (float)ft_atoi_type(mlx->scene[2], &i);
	mlx->cam_pos.y = (float)ft_atoi_type(mlx->scene[2], &i);
	mlx->cam_pos.z = (float)ft_atoi_type(mlx->scene[2], &i);
	if (mlx->scene[2][i] != ')')
		return (-1);
	if (ft_strncmp(mlx->scene[3], "	cam_dir(", 9) != 0)
		return (-1);
	i = 9;
	mlx->cam_dir.x = (float)ft_atoi_type(mlx->scene[3], &i);
	mlx->cam_dir.y = (float)ft_atoi_type(mlx->scene[3], &i);
	mlx->cam_dir.z = (float)ft_atoi_type(mlx->scene[3], &i);
	if (mlx->scene[3][i] != ')')
		return (-1);
