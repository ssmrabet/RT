...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/09/09 16:53:24 by smrabet           #+#    #+#             */
/*   Updated: 2017/09/09 16:53:32 by smrabet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_hud(t_mlx *mlx)
{
	...
	if (mlx->hud == 1)
	{
		...
		{
			s1 = ft_strjoin_type("anti-aliasing: x", ft_itoa(mlx->aa), 'R');
			MSP(mlx->mlx, mlx->win, 50, 735, 0x00FEDC, s1);
			free(s1);
		}
		s2 = ft_strjoin_type("number of reflections: ", ft_itoa(mlx->maxref),
			'R');
...
void		ft_instructions(void)
{
	...
	ft_putendl("B: Change image color Negatif/Sepia/Grayscale");
...
int			ft_objects_rot(int keycode, t_mlx *mlx)
{
	...
	if (keycode == LEFT && TMP->type != 4 && (tmp_rot = TMP->rot.x) < 6000.0)
	{
		TMP->rot.x = TMP->rot.x * cosf(0.5) + TMP->rot.z * sinf(0.5);
		TMP->rot.z = tmp_rot * (-sinf(0.5)) + TMP->rot.z * cosf(0.5);
	}
	if (keycode == RIGHT && TMP->type != 4 && (tmp_rot = TMP->rot.x) > -6000.0)
	{
		TMP->rot.x = TMP->rot.x * cosf(-0.5) + TMP->rot.z * sinf(-0.5);
		TMP->rot.z = tmp_rot * (-sinf(-0.5)) + TMP->rot.z * cosf(-0.5);
	}
	if (keycode == DOWN && TMP->type != 4 && (tmp_rot = TMP->rot.y) > -6000.0)
	{
		TMP->rot.y = TMP->rot.y * cosf(-0.5) + TMP->rot.z * (-sinf(-0.5));
		TMP->rot.z = tmp_rot * sinf(-0.5) + TMP->rot.z * cosf(-0.5);
	}
	if (keycode == UP && TMP->type != 4 && (tmp_rot = TMP->rot.y) < 6000.0)
	{
		TMP->rot.y = TMP->rot.y * cosf(0.5) + TMP->rot.z * (-sinf(0.5));
		TMP->rot.z = tmp_rot * sinf(0.5) + TMP->rot.z * cosf(0.5);
	}
